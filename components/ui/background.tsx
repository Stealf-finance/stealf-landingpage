...
export function ShaderBackground() {
  ...
  const camera = useMemo(() => ({ position: [0, 0, 1] as [number, number, number], fov: 90, near: 0.1, far: 1000 }), []);
  ...
  return (
    <div ref={canvasRef} className="fixed inset-0 w-full h-full bg-black" style={{ zIndex: 0 }} aria-hidden>
      <Canvas
        camera={camera}
