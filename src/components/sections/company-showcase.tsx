      ...
      <div className="grid w-full max-w-7xl grid-cols-2 md:grid-cols-3 overflow-hidden border-y border-border items-center justify-center z-20">
        {companyShowcase.companyLogos.map((logo) => (
          <Link
            ...
            target="_blank"
            rel="noreferrer"
            className="group w-full h-32 flex flex-col items-center justify-center relative p-4 gap-2 before:absolute before:-left-1 before:top-0 before:z-10 before:h-screen before:w-px before:bg-border before:content-[''] after:absolute after:-top-1 after:left-0 after:z-10 after:h-px after:w-screen after:bg-border after:content-['']"
            key={logo.id}
          >
            ...
            <span className="text-xs text-muted-foreground tracking-tight transition-all duration-300 ease-[cubic-bezier(0.165,0.84,0.44,1)] opacity-70 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0">
              {logo.role}
            </span>
